...
let roleMover = {
    /** @param {Creep} creep **/
    run: function(creep) {
        let homeRoom = creep.memory.homeRoom || (Game.spawns['Mammy'] ? Game.spawns['Mammy'].room.name : null);
        ...
        if (creep.memory.working) {
            ...
            if (allSources.length > 0) {
                for (let source of allSources) {
                    source.creepsGoing = creepMovement.countCreepsGoingToSource(source.id, creep.room.name, 'roleMover');
                }
            ...
            if (allContainers.length > 0) {
                ...
                for (let container of allContainers) {
                    ...
                    let nearbyMovers = creep.room.find(FIND_MY_CREEPS, {
                        filter: c => c.memory.role === 'roleMover' &&
                            c !== creep &&
