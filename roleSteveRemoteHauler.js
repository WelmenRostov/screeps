...
let roleSteveRemoteHauler = {
    countCreepsGoingToSource: function(sourceId, targetRoom) {
        ...
        if (cache.lastTick !== Game.time) {
            ...
            for (let name in Game.creeps) {
                ...
                if (c.memory.role === 'RemoteHauler' && c.memory.committedTargetId) {
    ...
    run: function(creep) {
        ...
        const homeRoom = creep.memory.homeRoom || (Game.spawns['Steve'] ? Game.spawns['Steve'].room.name : creep.room.name);
        ...
        if (!isFull) {
            ...
            let storage = null;
            let bobSpawn = Game.spawns['Steve'];
...
module.exports = roleSteveRemoteHauler;
